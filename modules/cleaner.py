...
class DataCleaner:
    ...
    def clean_all(self, products_df, stores_df, sales_df, inventory_df):
        ...
        self.stats['invalid_skus'] = fk_issues['invalid_skus']
        self.stats['invalid_stores'] = fk_issues['invalid_stores']
        
        return clean_products, clean_stores, clean_sales, clean_inventory
    
    def _update_stats(self, report):
        """Update overall stats from a report."""
        self.stats['missing_values_fixed'] += report.get('missing_fixed', 0)
        self.stats['duplicates_removed'] += report.get('duplicates_removed', 0)
        self.stats['outliers_fixed'] += report.get('outliers_fixed', 0)
        self.stats['negatives_fixed'] += report.get('negatives_fixed', 0)
        self.stats['text_standardized'] += report.get('text_standardized', 0)
        
        total_fixed = (
            report.get('missing_fixed', 0) +
            report.get('duplicates_removed', 0) +
            report.get('outliers_fixed', 0) +
            report.get('negatives_fixed', 0) +
            report.get('text_standardized', 0)
        )
        self.stats['total_issues_fixed'] += total_fixed
        self.stats['total_issues_found'] += total_fixed
    ...
    def get_cleaning_summary(self):
        """Return a summary of the cleaning operation."""
        return {
            'total_fixed': self.stats['total_issues_fixed'],
            'missing_values': self.stats['missing_values_fixed'],
            'duplicates': self.stats['duplicates_removed'],
            'outliers': self.stats['outliers_fixed'],
            'negatives': self.stats['negatives_fixed'],
            'text_standardized': self.stats['text_standardized'],
            'fk_issues': self.stats['invalid_skus'] + self.stats['invalid_stores']
        }
    
    def get_report(self):
        """Return the full cleaning report."""
